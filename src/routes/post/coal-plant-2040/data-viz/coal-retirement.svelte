<script>
    ...
    $: titleText = selectedScenario === 'goal'
    ...
    onMount(() => {
        ...
        const gridGroup = canvasGroup.append('g').attr('class', 'grid');
        gridGroup.call(
            d3.axisLeft(y)
                .ticks(5)
                .tickSize(-netchartWidth)
                .tickFormat('')
        );
...
<div id="chart-container">
    <div class="title-container">
        {#key titleText}
            <div 
                class="title" 
                in:fly={{ y: 8, duration: 500, delay: 150, easing: cubicOut }} 
                out:fly={{ y: -8, duration: 500, easing: cubicOut }}
            >
                {titleText}
            </div>
        {/key}
